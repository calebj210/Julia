...
"""
function getCorrectionWeights(g::Grid, idx::Vector{Int64}; α = 0, dir = 1 + 0im)
...
function getCorrections(g::Grid; α = 0, β = 0)
    ...
    bpr = getCorrectionWeights(g, bpci, α = α, dir =  1 + 0im)  # Basepoint right
    bpu = getCorrectionWeights(g, bpci, α = α, dir =  0 + 1im)  # Basepoint up
    bpl = getCorrectionWeights(g, bpci, α = α, dir = -1 + 0im)  # Basepoint left
    bpd = getCorrectionWeights(g, bpci, α = α, dir =  0 - 1im)  # Basepoint down

    # Corner corrections
    crr = getCorrectionWeights(g, bpci, dir =  1 + 0im)         # Corner right
    cru = getCorrectionWeights(g, bpci, dir =  0 + 1im)         # Corner up
    crl = getCorrectionWeights(g, bpci, dir = -1 + 0im)         # Corner left
    crd = getCorrectionWeights(g, bpci, dir =  0 - 1im)         # Corner down

    # Endpoint corrections
    epr = getCorrectionWeights(g, bpci, α = β, dir =  1 + 0im)  # Endpoint right
    epu = getCorrectionWeights(g, bpci, α = β, dir =  0 + 1im)  # Endpoint up
    epl = getCorrectionWeights(g, bpci, α = β, dir = -1 + 0im)  # Endpoint left
    epd = getCorrectionWeights(g, bpci, α = β, dir =  0 - 1im)  # Endpoint down

    return Corrections(bpci, bpu, bpd, bpl, bpr, cru, crd, crl, crr, epu, epd, epl, epr)
end
...
function getExternalWeights(zIdx, c::Corrections, g::Grid, α, β; branch = false)
    ...
    for (n, p) ∈ pairs(path)
        ...
        iIdx = getCorrectionIndices(p.i, c.bpci, g)                     # Initial point correction indices
        fIdx = getCorrectionIndices(p.f, c.bpci, g)                     # Final point correction indices
