...
function comp(N=100; n=10, m=3, o=n-1)
    ...
    maxErr = scalError(true∇∇F, laps)[1]
    ...
    #                markerstrokewidth = 0,
    #                markeralpha = 0.75,
    #                title = L"\textrm{LBO}\textrm{ for LBO on }\pi\textrm{, }p = %$o, n = %$n",
    #                xlabel = L"\mathrm{Re}(\lambda)",
    #                ylabel = L"\mathrm{Im}(\lambda)",
    #                legend = false,
    #                dpi = 300)

    # png(figA,"LBOLBOPiOdd$(o).png")
...
function lapErrs()
    P = [3:8...];
    nodes = 4000:4000:20000;
    a = plot()
    for p ∈ P
        # Compute even neighbor error
        err = [];
        for N ∈ nodes
            comps = comp(N, n = 2(p + 1), m = 7, o = p)
            push!(err,comps);
        end
        
        plot!(nodes,err,
              title = L"\textrm{LBO for LBO on }\pi",
              label = L"p = %$p, n = %$(2(p + 1))",
              legend = :bottomleft,
              xlabel = "# of Nodes",
        ...
        # Compute odd neighbor error
        err = [];
        for N ∈ nodes
            comps = comp(N, n = 2(p + 1) + 1, m = 3, o = p)
            push!(err,comps);
        end
        
        plot!(nodes,err,
              title = L"\textrm{LBO for LBO on }\pi",
              label = L"p = %$p, n = %$(2(p + 1) + 1)",
              style = :dash,
              legend = :bottomleft,
              xlabel = "# of Nodes",
              ylabel = L"||\mathrm{Error}\;||_\infty",
...
# function runTests()
#     for p = 3:8
#         comp(2000, n = 2(p + 1), m = 3, o = p)
#     end
# end

# Laplace-Beltrami
# comp(5000, n = 2(8+1) - 1, m = 7, o = 8)
