...
function constructLBD(nodes, n, m, typ, o)
    ...
    for i ∈ 1:N
        ...
        for i ∈ 1:n
            ...
            if typ == 1
                # Covarient derivative operator
                Lϕ[i] == s^(-1/2)*ϕ_x(0, xi, m);
            elseif typ == 2
...
function comp(N=100; n=10, m=3, o=n-1)
    ...
    # Construct Plot
    figA = scatter(real(λ), imag(λ),
                   markersize = 1,
                   markerstrokealpha = 0,
                   markerstrokewidth = 0,
                   markeralpha = 0.75,
                   title = L"\textrm{CDO}^2\textrm{LBO for BHO on }\pi",
                   xlabel = L"\mathrm{Re}(\lambda)",
                   ylabel = L"\mathrm{Im}(\lambda)",
                   legend = false,
                   dpi = 300)

    png(figA, "CDO2LBOBHOPi.png")
